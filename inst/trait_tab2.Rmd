...
austraits <- readRDS("C:/Users/z3524079/Documents/GitHub/austraits.portal/ignore/austraits.rds")

my_kable_styling <- util_kable_styling_html

trait <- "photosynthetic_pathway"
...
if(isTRUE(traits_numeric == trait)) {
  ...
  writeLines(
    sprintf("- **allowable range**: %s -  %s %s", trait_definition$allowed_values_min, trait_definition$allowed_values_max, trait_definition$units)
  )
...
if(isTRUE(traits_categorical == trait)) {
  ...
  most_common_families <- trait_data_tmp %>%
    select(-taxon_name) %>%
    group_by(family) %>%
    mutate(counts_per_value = sum(counts)) %>%
    ungroup() %>%
  ...
  plotdata1 <- trait_data_tmp %>%
    ...
    group_by(family, value) %>%
    mutate(counts_per_value = sum(counts)) %>%
    ungroup() %>%
    distinct(family, value, counts_per_value) %>%
    tidyr::pivot_wider(names_from = value, values_from = counts_per_value)
